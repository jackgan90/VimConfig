...
fun! s:setline(start, lines)
	"modified manually for encoding
	if exists('*Before_vc_setline')
		let s:convlines = Before_vc_setline(a:start, a:lines)
	else
		let s:convlines = a:lines
	endif
	"modification end
    try | let oul = &undolevels | catch | endt
    try | set undolevels=-1 
    catch | endt
	"The following line is also modified to fit our encoding need
    let lines = type(s:convlines) == type([]) && len(s:convlines) > 0 ? filter(s:convlines, 'len(v:val)>0') : s:convlines
	"modification end
